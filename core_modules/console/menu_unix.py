...
def _draw_text(stdscr, row: int, col: int, text: str, *, width: int | None = None, attrs: int = 0) -> None:
    ...
    try:
        stdscr.addstr(row, col, clipped, attrs)
    except curses.error:
        return
...
def prompt_text(label: str, initial: str = "", help_lines: list[str] | None = None) -> str | None:
    ...
    def _run(stdscr) -> str | None:
        ...
        while True:
            ...
            height, width = stdscr.getmaxyx()
            if _is_screen_too_small(height, width):
                _render_screen_too_small(stdscr, title=label, height=height, width=width)
                stdscr.refresh()
                key = stdscr.getch()
                if key == "\x1b":
                    return None
                continue
            _draw_text(stdscr, 0, 0, label, width=width - 1, attrs=curses.A_BOLD)
            for row, line in enumerate(help_lines or TEXT_HELP_LINES, start=1):
                _draw_text(stdscr, row, 0, line, width=width - 1, attrs=curses.A_BOLD)
            text = "".join(chars)
    ...
    def _run(stdscr):
        ...
        while thread.is_alive():
            ...
            if _is_screen_too_small(height, width):
                ...
                time.sleep(0.1)
                continue
            _draw_text(stdscr, 0, 0, title, width=width - 1, attrs=curses.A_BOLD)
            _draw_text(stdscr, 1, 0, "正在处理中，请稍候", width=width - 1, attrs=curses.A_BOLD)
            row = 3
            for line in lines:
                if row >= height:
                    break
                _draw_text(stdscr, row, 0, line, width=width - 1)
                row += 1
            stdscr.refresh()
            time.sleep(0.1)
        return None
...
def _render_inline_choice_menu_item(
    ...
    *,
    ...
    for choice_index, choice in enumerate(item.inline_choices):
        ...
        _draw_text(stdscr, row, start_x, padded, width=remaining, attrs=choice_attr)
...
def _render_status_lines(stdscr, status_lines: list[str], content_end: int, left: int, content_width: int, height: int) -> None:
    ...
    if divider_row <= content_end or divider_row < 0:
        return
    _draw_text(stdscr, divider_row, left, "─" * max(0, content_width), width=content_width, attrs=curses.A_DIM)
