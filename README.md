...
- OpenAI Codex、Claude Code：代码协作、重构与测试补强

## 功能特性

- 交互式控制台：基于 `curses` 的终端 UI，无需记忆复杂命令参数
...
当前项目会继续保留语雀附件相关代码与配置结构，但**默认不下载语雀附件**。原因是：

- 语雀官方 API 当前未提供附件导出能力
- 实际抓取附件链接时，常见情况是返回预览页或认证相关页面，而不是可直接保存的原始文件
- 因此当前导出策略是：**图片照常下载到本地，语雀附件保留原始链接**
...
使用此工具前，请先确认你的语雀账号权限情况。根据当前已知情况：

- 非语雀会员：未经过测试，暂不确定是否能够申请 Token以及调用 API。
...
程序启动后会进入交互式控制台，按提示完成 Token 配置、知识库选择和导出设置。
...
- `regenerate_md.py`：对已经导出的 `.lake` 文件批量重新生成 Markdown 的维护脚本

## 输出结果

导出结果默认位于 `output/` 目录，通常包含以下内容：
...
如果你主要关注测试、维护脚本或排障，请直接阅读下面的文档导航。

## 文档导航

除 `README.md` 外，其余项目文档统一整理在 `docs/` 目录。

- [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)
  面向维护者的架构说明，介绍模块职责、关键数据流和输出结构。
...
A: 图片失败时，先检查网络和代理设置，再查看 `output/.../export.log` 中对应文档的资源下载错误。语雀附件当前默认不会下载，本地 Markdown 中会保留原始附件链接。更详细的定位方法见 [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md)。

**Q: 某篇文档的 Markdown 转换效果不对？**

A: 优先查看导出日志中的转换警告；如果已经有 `.lake` 文件且只是转换逻辑更新了，可以执行 `python regenerate_md.py` 重新生成 Markdown。详见 [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md)。
