...
当前版本：`v0.4.3`（支持 macOS 和 Windows；Windows 需安装 `windows-curses`）
...
- 交互式控制台：基于 `curses` 的终端界面，减少命令行参数记忆负担
...
语雀官方 OpenAPI 未提供附件导出能力，因此附件下载按登录方式区分：
...
会员权限也会影响 Token 模式，使用前需要确认：

- 非语雀会员：未经过测试，暂不确定是否能够申请 Token 以及调用 API。
- 语雀专业会员：可申请 Token，但每天 API 可用额度较少；超出后语雀 API 会返回 `429`。
- 语雀超级会员：不受上述限制。
...
- 支持 macOS 和 Windows 运行。

## 运行要求

- Python >= 3.10
- macOS 或 Windows（Windows 需安装 `windows-curses`）
- 需要在支持 UTF-8 和交互式终端的环境中运行。Windows 使用 Windows Terminal 或新版 PowerShell。
...
Windows 首次运行前先安装控制台依赖。如果 `python` 不在 PATH 中，可改用 `py -3`：

```powershell
py -3 -m pip install windows-curses
py -3 yuque2markdown.py
```
...
- 当过滤词命中目录名时，会自动展开该目录并显示目录下文档。
- 当过滤词命中折叠目录内的文档时，上层目录路径也会自动显示出来。
...
浏览器 Cookie 自动读取仅支持 macOS 上的 Chromium 系浏览器。Windows 可在“浏览器 Cookie”登录方式下手动粘贴 Cookie。

1. 在浏览器中登录语雀。
2. 在控制台中将“登录方式”切换为“浏览器 Cookie”。
3. macOS 使用“从浏览器读取 Cookie”获取 Cookie；Windows 手动输入 Cookie 后刷新连接状态。
...
| `Esc` | 取消输入或清空过滤 |
| `s` | 保存配置 |
| `Esc`（非编辑状态） | 返回上一级或退出当前界面 |
| `Ctrl+C` | 在导出过程中触发退出确认 |

## 项目入口

- `yuque2markdown.py`：程序主入口，执行启动检查并进入交互式控制台
- `core_modules/auth/`：登录凭据处理，主要用于读取浏览器 Cookie
- `core_modules/console/`：终端菜单、状态展示和交互逻辑
  - `menu_unix.py`：macOS curses 菜单实现
  - `menu_windows.py`：Windows curses 菜单实现，依赖 `windows-curses`
  - `selector.py`：文档树选择器
...
A: 语雀 API 存在请求频率限制。程序会等待后重试；如果频繁触发，把“请求间隔”调到 `0.1` 秒或更高。
...
A: 请在真实终端中运行，并确保终端支持 UTF-8。程序不支持在非交互式终端中启动 `curses` 界面；Windows 下请先安装 `windows-curses`。若终端窗口过小，程序会提示最低窗口尺寸要求，不继续渲染菜单。
