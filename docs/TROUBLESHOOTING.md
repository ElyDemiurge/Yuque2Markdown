...
- `output/<知识库>/regenerate.log`
  ...
  如果本地已有 `assets/`，脚本会复用这些图片并改写 Markdown 链接。
...
- 如果某本知识库在列表里是灰色的，说明它不属于当前登录账号，不能导出。
- 如果知识库列表里能移动但无法确认，先确认本地版本是否已包含 Enter 兼容修复；代码应同时兼容 `getch()` 与 `get_wch()` 的 Enter 返回形式。
...
1. 使用 Windows Terminal 或新版 PowerShell。
2. 确认终端字体支持中文显示。
3. 程序启动时会把标准输入输出切到 UTF-8；如果仍然异常，可在启动前执行：
...
4. 如果 `python` 不在 PATH 中，使用 `py -3` 运行安装和测试命令。
...
- 是否触发了限流，而不是鉴权失败
- 如果不是在 macOS 或 Windows 上运行，启动时会提示平台不支持；Windows 缺少 `windows-curses` 时会提示安装该依赖
...
4. 如果行为与上述预期不一致，再回看对应平台的 `core_modules/console/menu_*.py` 和 `core_modules/console/selector.py`。
...
- Lake 相关问题先写最小复现测试，不要直接依赖完整导出样本。
...
只有在确认断点文件异常、且接受重新导出时，再删除它。默认保留。
