...
Lake 是语雀编辑器的内部文档格式，以 XML 形式存储在 API 响应的 `body_lake` 字段中。本工具通过解析 `body_lake` 将文档转换为 Markdown。
...
Lake 中的图片、代码块等富媒体不直接用 HTML 标签，而是用 `<card>` 包裹 URL 编码的 JSON 数据。
...
以下类型已识别但会产生警告 `Lake 转换未完全覆盖：存在未支持 card 类型 <type>`：
...
| 场景 | 警告内容 |
|------|----------|
| `body_lake` 为空 | `文档缺少 body_lake 内容` |
| `body_lake` 空但有 `body` | `文档缺少 body_lake 内容，使用 body 字段代替（可能丢失部分格式）` |
| XML 解析失败 | `lake 文档解析失败，请结合 .lake 文件检查` |
| card 解码失败 | `Lake card 解码失败: <type>` |
| card value 解码失败 | `Lake card value 无法解码: <type>` |
| 未知 card 类型 | `Lake 转换未完全覆盖：存在未支持 card 类型 <type>` |
| 残留标签检测 | `Lake 转换后仍残留原始标签，请结合 .lake 文件检查` |
| 表格标签 | `Lake 转换遇到未专门处理的标签: <td/tr/tbody/table>` |
| 通用未处理标签 | `Lake 转换遇到未专门处理的标签: <xxx>` |
| 通用行内未处理标签 | `Lake 行内转换遇到未专门处理的标签: <xxx>` |
...
- `core_modules/lake/localizer.py`：资源离线化
- `core_modules/lake/models.py`：数据类型定义

## 相关文档

- [../README.md](../README.md)：项目总览与使用入口
- [ARCHITECTURE.md](ARCHITECTURE.md)：模块结构与关键数据流
- [TESTING.md](TESTING.md)：测试覆盖与回归建议
