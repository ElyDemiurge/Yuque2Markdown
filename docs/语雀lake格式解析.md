...
Lake 里的图片、代码块等富文本内容不直接写成 HTML 标签，而是放在 `<card>` 节点里，内容是 URL 编码后的 JSON。
...
`table` card 会先读取其中的 HTML 表格，再降级转换为 Markdown 表格。
...
`board` / `lakeboard` 会先提取思维导图文本大纲，再保留整图链接。
...
> - `lockedtext` 属于加密内容，转换器会静默跳过，不写入正文，也不追加警告。
...
- [TESTING.md](TESTING.md)：测试覆盖与补测方式
