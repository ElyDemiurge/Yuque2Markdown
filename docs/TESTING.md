...
统一使用 pytest 运行测试，不再依赖测试文件底部的兼容入口；主要测试文件已移除这类手写入口。
...
2. 文件名使用 `test_*.py`。
3. 函数名使用 `test_*`。
4. 使用 pytest 风格测试，不新增额外的测试框架依赖。
5. 新增功能或修复 bug 时，同步补相关测试。
6. Lake 卡片或 Markdown 转换问题使用最小可复现输入构造测试，不依赖完整导出样本。
7. 导出流程问题使用 `FakeClient` 这类测试替身构造稳定场景，避免真实网络请求。

## 编写测试原则

- 测试最终结果，不要过度绑定内部实现细节。
- 路径、链接改写、日志文本这类行为，断言最终输出。
- Lake 转换问题断言生成 Markdown 中的关键片段和警告内容。
- 配置校验问题断言错误消息和边界输入是否被接受。
