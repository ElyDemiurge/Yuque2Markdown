...
- 下载图片资源并改写为本地链接，Cookie 模式下可按扩展名下载语雀附件
- 记录日志与断点，支持中断恢复和问题排查
...
- `regenerate_md.py`
  维护脚本。遍历 `output/` 中已有的 `.lake` 与 `.yuque.json`，重新生成对应 Markdown；本地已有 `assets/` 时复用，缺失资源且配置里有可用凭据时再补充下载。
- `pytest.ini`
...
- `localizer.py`：图片和附件下载、本地路径生成、链接替换，以及语雀附件保留方式
...
- 重新调用 `build_doc_markdown_result()` 生成 Markdown
- 将结果覆盖写回对应 `.md`
